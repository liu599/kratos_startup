...
var PathDemoSayHelloURL = "/small-service/say_hello"
var PathDemoRequestItem = "/small-service/request"
...
type DemoBMServer interface {
	...
	RequestItem(ctx context.Context, req *HelloReq) (resp *HelloResp, err error)
}
...
func demoRequestItem(c *bm.Context) {
	p := new(HelloReq)
	if err := c.BindWith(p, binding.Default(c.Request.Method, c.Request.Header.Get("Content-Type"))); err != nil {
		return
	}
	resp, err := DemoSvc.RequestItem(c, p)
	c.JSON(resp, err)
}
...
func RegisterDemoBMServer(e *bm.Engine, server DemoBMServer) {
	...
	e.GET("/small-service/say_hello", demoSayHelloURL)
	e.GET("/small-service/request", demoRequestItem)
}
